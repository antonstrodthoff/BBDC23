...
def plotColumn(fileName="", plotColumn=2, separator=";", dropFirstNValues=1, dateFormat="%d.%m.%Y", label="", window=0, alpha=1):
    ...
    if window == 0:
        ...
        data.iloc[:, plotColumn] = data.iloc[:, plotColumn].replace(0, np.nan).astype(float).interpolate(limit_direction="both").rolling(window=window, min_periods=window//2, center=True).mean()
    ...
    plt.plot(xx[15000:], data.iloc[15000:, plotColumn], linewidth=0.5, alpha=alpha, label=label)
...
plotColumn(fileName="task_student/bbdc_2023_AWI_data_develop_student.csv", plotColumn=7, separator=";", dropFirstNValues=1, label="t_nox", window=100)

#plotColumn(fileName="task_student/bbdc_2023_AWI_data_develop_student.csv", plotColumn=2, separator=";", dropFirstNValues=1, label="t_secci_inter", window=50)
